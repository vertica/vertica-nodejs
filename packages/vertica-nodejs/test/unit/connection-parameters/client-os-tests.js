...
suite.test('client_os provides detailed OS string', function () {
  ...
  const value = subject.client_os

  assert.ok(value.includes(os.type()), `Expected "${value}" to include os.type()`)
  assert.ok(value.includes(os.release()), `Expected "${value}" to include os.release()`)
  assert.ok(value.includes(os.arch()), `Expected "${value}" to include os.arch()`)
})
